...
export default function RequestHelpModal({ targetUser, onClose, onRequestCreated }) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        alert(`Help request sent successfully to ${targetUser.firstName} ${targetUser.lastName}!`);
        onClose();
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-purple-200 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Request help from {targetUser.firstName} {targetUser.lastName}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              className="flex-1 px-4 py-2 rounded-xl bg-gradient-to-r from-[#7D4DF4] to-[#A589FD] text-white shadow hover:opacity-90 transition disabled:opacity-50"
              disabled={isSubmitting}
            >
